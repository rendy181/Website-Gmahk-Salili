...
interface ChurchContextType {
  ...
  addChapter: (chapter: Omit<BookChapter, 'id'>) => Promise<boolean>;
  updateChapter: (chapter: BookChapter) => Promise<boolean>;
  deleteChapter: (id: number) => Promise<boolean>;
  getChaptersByBook: (bookId: number) => BookChapter[];
...
export const ChurchProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadSupabaseData = useCallback(async () => {
    ...
    try {
      ...
      tableResults['book_chapters'] = !chaptersErr;
  ...
  const updatePastoralContact = async (contact: Partial<PastoralContact>): Promise<boolean> => {
    try {
      const updated = { 
        ...
        whatsappnumber: contact.whatsappNumber || pastoralContact.whatsappNumber || '',
        welcomemessage: contact.welcomeMessage || pastoralContact.welcomeMessage || '',
  ...
  const getChaptersByBook = (bookId: number): BookChapter[] => {
    return bookChapters.filter((c) => c.book_id === bookId);
  };

  // === BOOK CHAPTERS ===
  const addChapter = async (chapter: Omit<BookChapter, 'id'>): Promise<boolean> => {
    try {
      const { data, error } = await supabase
        .from('book_chapters')
        .insert([chapter])
      ...
      if (error) throw error;
      if (data) {
        setBookChapters([...bookChapters, data[0]]);
      }
      await refreshData();
      return true;
    } catch (e) {
  ...
  const updateChapter = async (chapter: BookChapter): Promise<boolean> => {
    try {
      await supabase.from('book_chapters').update(chapter).eq('id', chapter.id);
      setBookChapters(bookChapters.map((c) => (c.id === chapter.id ? chapter : c)));
      await refreshData();
      return true;
    } catch (e) {
  ...
  const deleteChapter = async (id: number): Promise<boolean> => {
    try {
      ...
      setBookChapters(bookChapters.filter((c) => c.id !== id));
      await refreshData();
      return true;
    } catch (e) {
