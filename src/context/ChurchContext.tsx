  ...
  const loadSupabaseData = useCallback(async () => {
    ...
    try {
      ...
      if (pastData && pastData.length > 0) {
        setPastoralContact({ ...DEFAULT_PASTORAL_CONTACT, ...pastData[0] });
      }
...
const addArticle = async (art: Omit<Article, 'id'>): Promise<boolean> => {
  try {
    ...
    // Kirim SEMUA data termasuk penulis dan kategori
    const { data, error } = await supabase
      .from('articles')
      .insert([{
        judul: art.judul,
        ...
        slug: slug,
        penulis: art.penulis || 'Admin',   // ← PASTIKAN INI ADA!
        kategori: art.kategori || 'Rohani' // ← PASTIKAN INI ADA!
      }])
      .select();
